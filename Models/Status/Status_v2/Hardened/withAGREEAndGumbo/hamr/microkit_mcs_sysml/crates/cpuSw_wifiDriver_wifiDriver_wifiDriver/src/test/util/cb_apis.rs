...
use crate::bridge::cpuSw_wifiDriver_wifiDriver_wifiDriver_GUMBOX as GUMBOX;
  ...
  api_wifiRecv: Option<Common::IncomingWifiMessage_Impl>) -> HarnessResult
{
  ...
  if !GUMBOX::compute_CEP_Pre (In_alert_cmd, api_alert_in, api_analysis_report, api_wifiRecv) {
    return HarnessResult::RejectedPrecondition;
  }

  // [PutInPorts]: Set values on the input ports
  put_alert_in(api_alert_in);
  ...
  // [CheckPost]: invoke the oracle function
  if !GUMBOX::compute_CEP_Post(In_alert_cmd, alert_cmd, api_alert_in, api_analysis_report, api_wifiRecv, api_HMD_log, api_analysis_request, api_wifiSend) {
...
{
  return testComputeCB(container.api_alert_in, container.api_analysis_report, container.api_wifiRecv)
}
...
testComputeCB_macro {
  (
    $test_name: ident,
    config: $config:expr,
    api_alert_in: $api_alert_in_strat:expr,
    ...
    proptest!{
      ...
      #[serial]
      fn $test_name(
        (api_alert_in, api_analysis_report, api_wifiRecv)
            in ($api_alert_in_strat, $api_analysis_report_strat, $api_wifiRecv_strat)
      ) {
        match$crate::test::util::cb_apis::testComputeCB(api_alert_in, api_analysis_report, api_wifiRecv) {
          $crate::test::util::cb_apis::HarnessResult::RejectedPrecondition => {
...
{
  return testComputeCBwGSV(container.In_alert_cmd, container.api_alert_in, container.api_analysis_report, container.api_wifiRecv)
}
...
testComputeCBwGSV_macro {
  (
    ...
    proptest!{
      ...
      #[serial]
      fn $test_name(
        (In_alert_cmd, api_alert_in, api_analysis_report, api_wifiRecv)
            in ($In_alert_cmd_strat, $api_alert_in_strat, $api_analysis_report_strat, $api_wifiRecv_strat)
      ) {
        match $crate::test::util::cb_apis::testComputeCBwGSV(In_alert_cmd, api_alert_in, api_analysis_report, api_wifiRecv) {
          $crate::test::util::cb_apis::HarnessResult::RejectedPrecondition => {
            return Err(proptest::test_runner::TestCaseError::reject(
              "Precondition failed: invalid input combination",
            ))
