...
use crate::bridge::cpuSw_firewall_firewall_GUMBOX as GUMBOX;
...
/** Contract-based test harness for the compute entry point
  ...
  */
pub fn testComputeCB(api_analysis_request_in: Option<Common::AnalysisRequest_Impl>) -> HarnessResult
{
  // Initialize the app
  crate::cpuSw_firewall_firewall_initialize();

  // [PutInPorts]: Set values on the input ports
  put_analysis_request_in(api_analysis_request_in);

  // [InvokeEntryPoint]: Invoke the entry point
  crate::cpuSw_firewall_firewall_timeTriggered();

  // [RetrieveOutState]: retrieve values of the output ports via get operations and GUMBO declared local state variable
  let api_analysis_request_out = get_analysis_request_out();

  // [CheckPost]: invoke the oracle function
  if !GUMBOX::compute_CEP_Post(api_analysis_request_in, api_analysis_request_out) {
...
{
  return testComputeCB(container.api_analysis_request_in)
}
...
testComputeCB_macro {
  (
    $test_name: ident,
    config: $config:expr,
    api_analysis_request_in: $api_analysis_request_in_strat:expr
  ) => {
    proptest!{
      #![proptest_config($config)]
      #[test]
      #[serial]
      fn $test_name(
        (api_analysis_request_in)
            in ($api_analysis_request_in_strat)
      ) {
        match$crate::test::util::cb_apis::testComputeCB(api_analysis_request_in) {
          $crate::test::util::cb_apis::HarnessResult::RejectedPrecondition => {
